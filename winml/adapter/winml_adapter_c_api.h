...
enum OrtProfilerEventCategory { SESSION_EVENT = 0, NODE_EVENT, EVENT_CATEGORY_MAX };
...
struct WinmlAdapterApi {
   ...
   * There is no inferencing or evaluation setup performed. Only ONNX load is done to reflect on the model's
   * inputs/outputs and other properties. This is used by WinML to support model reflection APIs.
   */
  ...
  /**
   * ModelGetInputCount
   * This api gets the number of inputs from the OrtModel. It closely matches the API of a similar name similar name for
   * retrieving model metadata from OrtSession. This is used by WinML to support model reflection APIs.
   */
  OrtStatus*(ORT_API_CALL* ModelGetInputCount)(_In_ const OrtModel* model, _Out_ size_t* count)NO_EXCEPTION;

  /**
   * ModelGetOutputCount
   * This api gets the number of outputs from the OrtModel. It closely matches the API of a similar name for retrieving
   * model metadata from OrtSession. This is used by WinML to support model reflection APIs.
   */
  OrtStatus*(ORT_API_CALL* ModelGetOutputCount)(_In_ const OrtModel* model, _Out_ size_t* count)NO_EXCEPTION;

  /**
   * ModelGetInputName
   * This api gets the input name from the OrtModel given an index. It closely matches the API of a similar name for
   * retrieving model metadata from OrtSession. This is used by WinML to support model reflection APIs.
   */
  ...
  /**
   * ModelGetOutputName
   * This api gets the output name from the OrtModel given an index. It closely matches the API of a similar name for
   * retrieving model metadata from OrtSession. This is used by WinML to support model reflection APIs.
   */
   ...
   * This api gets the input description from the OrtModel given an index. It closely matches the API of a similar name
   * for retrieving model metadata from OrtSession. This is used by WinML to support model reflection APIs.
   */
   ...
   * This api gets the output description from the OrtModel given an index. It closely matches the API of a similar name
   * for retrieving model metadata from OrtSession. This is used by WinML to support model reflection APIs.
   */
   ...
   * This api gets the input OrtTypeInfo from the OrtModel given an index. It closely matches the API of a similar name
   * for retrieving model metadata from OrtSession. This is used by WinML to support model reflection APIs.
   */
   ...
   * This api gets the output OrtTypeInfo from the OrtModel given an index. It closely matches the API of a similar name
   * for retrieving model metadata from OrtSession. This is used by WinML to support model reflection APIs.
   */
   ...
   * This is used by WinML to deduce whether model input and output formats are supported by the WinML tensorization
   * code paths.
   */
   ...
   *
   * Can this API be moved into the EP during session initialization. Currently we do an early fp16 check to avoid
   * initialization when it is not supported.
   */
   ...
   * This api is used to create a Session that is completely uninitialized. While there are other Session creation APIs
   * in the c-abi, WinML uses this so that it can perform optimizations prior to loading the model, and initializing.
   * Moreover, WinML needs a new api to support the OrtModel type, and prevent the parsing model protobufs again on
   * session creation.
   */
  ...
  /**
   * SessionInitialize
   * This api is used to initialize an OrtSession. This is one component of creating a usable OrtSession, and is a part
   * of CreateSession in the c-abi. Currently WinML uses this to finalize session creation, after configuring a variety
   * of properties on the OrtSession.
   */
   ...
   *
   * Ideally these transformations should be configured by the contract between the runtime and the EP and not
   * overridden by WinML.
   */
   ...
   * This api is used to start profiling OrtSession. The existing mechanism only allows configuring profiling at session
   * creation.
   *
  ...
  /**
   * SessionEndProfiling
   * This api is used to end profiling OrtSession. The existing mechanism only allows configuring profiling at session
   * creation.
   *
   * WinML uses this to toggle profilling on and off based on if a telemetry providers are being listened to.
   */
   ...
   * This api returns false if the ort session options config entry "session.intra_op.allow_spinning" is set to "0", and
   * true otherwise
   *
   ...
   *
   * WinML uses this when graphs are evaluated with DML, and their outputs remain on the GPU but need to be copied back
   * to the CPU.
   */
   ...
   *
   * WinML uses this to manage caller specified D3D12 inputs/outputs. It uses the memory info here to call
   ...
   *
   * WinML uses this to create tensors, and needs to hold onto the allocator for the duration of the associated value's
   * lifetime.
   */
