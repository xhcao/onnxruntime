...
ORT_API_STATUS_IMPL(
  ...
  if (options) {
    if (!options->custom_op_domains_.empty()) {
      ...
      if (!status.IsOK()) return onnxruntime::ToOrtStatus(status);
    }
  }
...
ORT_API_STATUS_IMPL(winmla::SessionRegisterGraphTransformers, _In_ OrtSession* session) {
  ...
  // Bug 22973884 : Fix issues with BatchNorm + Add and BatchNorm + Mul handling implicit inputs, and move from Winml to
  // ORT
