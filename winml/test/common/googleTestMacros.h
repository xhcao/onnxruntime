...
#define WINML_TEST(group_name, test_name) \
  TEST_F(group_name, test_name) { getapi().test_name(); }
...
#define WINML_TEST_CLASS_SETUP_CLASS(setup_class) \
 protected:                                       \
  static void SetUpTestSuite() { getapi().setup_class(); }

#define WINML_TEST_CLASS_TEARDOWN_CLASS(teardown_class) \
 protected:                                             \
  static void TearDownTestSuite() { getapi().teardown_class(); }

#define WINML_TEST_CLASS_SETUP_METHOD(setup_method) \
 protected:                                         \
  void SetUp() override { getapi().setup_method(); }

#define WINML_TEST_CLASS_TEARDOWN_METHOD(teardown_method) \
 protected:                                               \
  void TearDown() override { getapi().teardown_method(); }
