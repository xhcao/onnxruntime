...
namespace WinML {
// Global needed to keep the actual ITestCase alive while the tests are going on. Only ITestCase* are used as test
// parameters.
...
std::string GetTestDataPath() {
  ...
  if (environmentVariableFetchSuceeded == 0 && GetLastError() == ERROR_ENVVAR_NOT_FOUND ||
    ...
    if (std::filesystem::exists(hardcodedModelPath) && hardcodedModelPath.length() <= MAX_PATH) {
      ...
      std::string errorStr =
          "WINML_TEST_DATA_PATH environment variable path not found and \"models\" folder not found in same directory "
          "as test exe.\n";
      std::cerr << errorStr;
  ...
  if (MAX_PATH - environmentVariableFetchSuceeded >= testDataPathFolderName.length()) {
    ...
    throw std::exception(
        "WINML_TEST_DATA_PATH environment variable path needs to be shorter to accomodate the maximum path size of "
        "%d\n",
        MAX_PATH
    );
  }
