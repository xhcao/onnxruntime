...
static void VerifyInvalidBindExceptions() {
  ...
  */
  ...
  // WINML_EXPECT_THROW_SPECIFIC(imageBinding.Bind(inputName, abiSequence), winrt::hresult_error,
  // ensureWinmlInvalidBinding);

  // // Bind invalid tensor type as image input
  // WINML_EXPECT_THROW_SPECIFIC(imageBinding.Bind(inputName, tensorBoolean), winrt::hresult_error,
  // ensureWinmlInvalidBinding);

  // // Bind invalid tensor size as image input
  // auto tensorFloat = TensorFloat::Create(std::vector<int64_t> { 1, 1, 100, 100 });
  // WINML_EXPECT_THROW_SPECIFIC(imageBinding.Bind(inputName, tensorFloat), winrt::hresult_error,
  // ensureWinmlInvalidBinding);

  // // Bind invalid tensor shape as image input
  // WINML_EXPECT_THROW_SPECIFIC(imageBinding.Bind(inputName, tensorInvalidShape), winrt::hresult_error,
  // ensureWinmlInvalidBinding);
