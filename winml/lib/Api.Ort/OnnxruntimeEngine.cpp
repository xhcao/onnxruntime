...
OrtValue* OnnxruntimeValue::UseOrtValue() { return value_.get(); }
...
OrtSession* OnnxruntimeEngine::UseOrtSession() { return session_.get(); }

const OrtApi* OnnxruntimeEngine::UseOrtApi() { return engine_factory_->UseOrtApi(); }

OnnxruntimeEngineFactory* OnnxruntimeEngine::GetEngineFactory() { return engine_factory_.Get(); }
...
HRESULT OnnxruntimeEngine::CreateTensorValueFromDefaultAllocator(
  ...
  OrtAllocator* ort_allocator;
  RETURN_HR_IF_NOT_OK_MSG(
      ort_api->GetAllocatorWithDefaultOptions(&ort_allocator), ort_api
 ...
 * its source location to the ort value. Since a copy is required, there is need to preserve the caller's memory
 * locations. We simply allocate memory with ORT and copy the tensorized values into it.
 */
HRESULT OnnxruntimeEngine::CreateTensorValue(
    const int64_t* shape, size_t count, winml::TensorKind kind, _Out_ IValue** out
) {
  auto ort_api = engine_factory_->UseOrtApi();
  auto winml_adapter_api = engine_factory_->UseWinmlAdapterApi();
  ...
  OrtAllocator* ort_allocator;
  RETURN_HR_IF_NOT_OK_MSG(
      winml_adapter_api->GetProviderAllocator(session_.get(), ort_provider, &ort_allocator),
      engine_factory_->UseOrtApi()
  );
  ...
  auto unique_value = UniqueOrtValue(ort_value, ort_api->ReleaseValue);
  RETURN_IF_FAILED(Microsoft::WRL::MakeAndInitialize<OnnxruntimeValue>(
      out, this, std::move(unique_value), std::move(unique_allocator)
  ));
  return S_OK;
}
 ...
 * WinML cannot use ORT allocators here since they will allocate the ID3D12Resource and force a copy from the user
 * provided value.
 */
HRESULT OnnxruntimeEngine::CreateTensorValueFromExternalD3DResource(
  ...
  RETURN_HR_IF_NOT_OK_MSG(
      ort_dml_api->CreateGPUAllocationFromD3DResource(d3d_resource, &dml_allocator_resource),
      engine_factory_->UseOrtApi()
  );
...
HRESULT OnnxruntimeEngine::CreateTensorValueFromExternalBuffer(
  ...
  if (kind == winml::TensorKind::String) {
    // String buffers cannot be passed into the ort api directly because ort c-api tensor strings cannot be backed by
    // external memory
    return E_NOTIMPL;
  }
...
STDMETHODIMP OnnxruntimeEngineFactory::CreateModel(
...
const OrtApi* OnnxruntimeEngineFactory::UseOrtApi() { return ort_api_; }

const WinmlAdapterApi* OnnxruntimeEngineFactory::UseWinmlAdapterApi() { return winml_adapter_api_; }
...
OnnxruntimeThreading::OnnxruntimeThreading()
    : inter_op_ort_pool_(nullptr, nullptr), intra_op_ort_pool_(nullptr, nullptr) {}
...
OrtThreadPool* OnnxruntimeThreading::UseIntraOpThreadPool() { return intra_op_ort_pool_.get(); }

OrtThreadPool* OnnxruntimeThreading::UseInterOpThreadPool() { return inter_op_ort_pool_.get(); }
