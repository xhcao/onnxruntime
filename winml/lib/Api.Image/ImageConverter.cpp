...
void ImageConverter::ResetAllocator() { WINML_THROW_IF_FAILED(command_allocator_->Reset()); }
...
ComPtr<ID3D11Texture2D> ImageConverter::CreateTextureFromUnsupportedColorFormat(
  ...
  // Make sure we create the new video frame on the same device. We don't want the VideoFrame pipeline to implicitly
  // share the texture between 2 devices since we will need to do it ourselves anyway.
