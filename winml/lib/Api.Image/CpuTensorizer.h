...
#pragma warning(push)
#pragma warning(disable : 26014)  // warning about possible out of bounds accesing pData, but input is checked for BGRA8
                                  // format, so uiCapacity should be in multiples of 4
    ...
    // bufferWidth may have padding because of optimization, but bytesPerRow includes only the real tensor data. We need
    // to jump over bufferWidth's extra padding
