...
void VideoFrameToTensorConverter::VideoFrameToDX12Tensor(
  ...
  WINML_THROW_HR_IF_FALSE_MSG(
      ...
      "Target tensor description must either be kImageTensorDataTypeFloat32, or kImageTensorDataTypeFloat16. %d was "
      "supplied.",
      tensorDesc.dataType
  );
  WINML_THROW_HR_IF_FALSE_MSG(
  ...
  if (inputVideoFrame.SoftwareBitmap()) {
    ...
    // TODO: Scale during the tensorization phase instead of using the video frame pipeline when the input bounds are
    // not the same size as the tensor
    if (!_winmli::DirectXPixelFormatSupported(spDirect3DSurface.Description().Format) ||
      ...
      // Force the VideoFrame to not do a conversion if the format is supported since we do it during the tensorization
      // anyway
      ...
      // If the color format is known or the input widths are not smaller than the tensor desc, just use the video frame
      // as is
...
void VideoFrameToTensorConverter::ConvertDX12TextureToGPUTensor(
  ...
  // Write compute commands into the command list and put it into the queue.
  {
    ...
    // This code currently re-uses the same decriptors each execution, which is unsafe if previous executions are in
    // flight.
...
void VideoFrameToTensorConverter::ConvertSoftwareBitmapToGPUTensor(
  ...
  // TODO: Scale during the tensorization phase instead of using the video frame pipeline when the input bounds are not
  ...
  // We avoid the Video Frame pipeline by manually sending the CPU data to the GPU, and we tensorize while we are
  // filling the upload heap. The image may already have been cropped/scaled by the video frame pipeline, so we send the
  // scaled bounds instead of the initial input bounds
...
void VideoFrameToTensorConverter::ConvertSoftwareBitmapToCPUTensor(
  ...
  WINML_THROW_HR_IF_FALSE_MSG(
      ...
      "Target tensor description expects kImageTensorChannelTypeGRAY8, kImageTensorChannelTypeBGR8, or "
      "kImageTensorChannelTypeRGB8 but has %d was specified.",
