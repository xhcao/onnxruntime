...
numeric_data::numeric_data(
  ...
  if (buffers_.size() == 0) {
    ...
    // If there are many buffers, then the combined buffer will be a separately allocated value that combines all of the
    // buffers. This needs to be lazily done however, as the extra memory should not be allocated when not needed (GPU).
  }
}

size_t numeric_data::num_elements() { return num_elements_; }

size_t numeric_data::size_in_bytes() { return num_elements_ * element_size_in_bytes_; }

size_t numeric_data::num_buffers() { return buffers_.size(); }

std::vector<wss::IBuffer>& numeric_data::buffers() { return buffers_; }
...
gsl::span<byte> numeric_data::buffer_at(size_t index) { return get_span_from_ibuffer(buffers_[index]); }
