...
namespace WINMLP {
LearningModelBinding::~LearningModelBinding() { Clear(); }
...
uint32_t LearningModelBinding::Size() { return static_cast<uint32_t>(m_providers.size()); }
...
void LearningModelBinding::Split(
  ...
  // If the IMapView instance cannot be split, then both the first and second parameters are null when the method
  // returns.
  first = nullptr;
  second = nullptr;
}
...
const std::vector<std::string>& LearningModelBinding::GetOutputNames() const { return output_names_; }

const std::vector<std::string>& LearningModelBinding::GetInputNames() const { return input_names_; }

std::vector<winrt::com_ptr<_winml::IValue>>& LearningModelBinding::GetOutputs() { return outputs_; }

const std::vector<winrt::com_ptr<_winml::IValue>>& LearningModelBinding::GetInputs() const { return inputs_; }
