...
// This class holds onto the lifetime of TensorResources<T> so that they can be kept alive by TensorBase AND its active
// MBRs. When the last MBR/Tensor object is destroyed then TensorResources<T> and its associated cpu and gpu resources
// will be destroyed. The source MB (the tensor object) holds weak references to its TensorMemoryBufferReference<T> MBRs
...
class TensorMemoryBufferReference : public winrt::implements<
 ...
 public:
  ...
  uint32_t Capacity() const try {
    ...
    // (https://docs.microsoft.com/en-us/uwp/api/windows.foundation.imemorybuffer.createreference) If the
    // IMemoryBufferReference has been closed (m_tensorResources == nullptr) then "IMemoryBufferReference instance's
    // Capacity property will be zero."
  ...
  (_Outptr_result_buffer_(*capacity) BYTE** value, _Out_ UINT32* capacity) try {
    ...
    *capacity = 0;
    ...
    // Per IMemoryBuffer.CreateReference
    // (https://docs.microsoft.com/en-us/uwp/api/windows.foundation.imemorybuffer.createreference) If the
    // IMemoryBufferReference has been closed (m_tensorResources == nullptr) then "IMemoryBufferByteAccess::GetBuffer
    // method will always return a null memory pointer and zero capacity."
