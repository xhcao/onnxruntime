...
struct TensorBase : TBase {
    ...
    if (CpuTensor() != nullptr) {
      ...
      if (num_backing_buffers == 1) {
        ...
        if (context.type == _winml::BindingType::kInput) {
          ...
          // If we are binding outputs, then the buffers do not need to bound. If the engine produces a output on the
          // gpu we already have a detensorize path which will need to copy the allocated output d3d12resource into the
          // output buffers without temporary intermediary buffers! No binding here is necessary. If the output produces
          // a cpu buffer (even in the GPU case), we will already have a cpu buffer, and just need to copy back to the
          // output buffers, no binding is necessary.
          GpuTensor() = CreateD3D12Resource(session);
          return CreateGPUMLValue(GpuTensor().get(), context, out);
        }
      }
  ...
  (BindingContext& context, IValue* value) {
    ...
    if (isCpuOutput) {
      ...
      // resources for tensors. Therefore we are certain that the returned dxresource is the same as the one we passed
      // in and was updated in place.
      auto session = context.session.as<winmlp::LearningModelSession>();
      auto device = session->Device().as<winmlp::LearningModelDevice>();
      auto engine = session->GetEngine();
      ...
      if (CpuTensor()->num_buffers() == 1) {
        ...
        // Reset the Allocator before return to the Cache. Must Sync this background thread to that completion before we
        // do.
  ...
  wf::IMemoryBufferReference CreateReference() try {
    ...
    // Per IMemoryBuffer.CreateReference
    // (https://docs.microsoft.com/en-us/uwp/api/windows.foundation.imemorybuffer.createreference) "This method always
    // successfully returns a new IMemoryBufferReference object even after the IMemoryBuffer "has been closed. In that
    // case, the returned IMemoryBufferReference is already closed." Creating a TensorMemoryBufferReference<T> with a
    // null pointer is equivalent to creating it as closed.
  ...
  wfc::IVectorView<ElementViewType> GetAsVectorView() try {
    ...
    //   2) the signature of the method conforms to the ABI signature and the return value matches the ABI Return Type
    //   (ViewT).
