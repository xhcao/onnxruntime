...
void WinMLTelemetryHelper::SetIntraOpNumThreadsOverride(uint32_t num_threads_override) {
  if (!telemetry_enabled_) return;
  WinMLTraceLoggingWrite(
      provider_,
...
void WinMLTelemetryHelper::SetIntraOpThreadSpinning(bool allow_spinning) {
  if (!telemetry_enabled_) return;
  WinMLTraceLoggingWrite(
      provider_,
...
void WinMLTelemetryHelper::SetLearningModelDeviceKind(int device_kind) {
  if (!telemetry_enabled_) return;
  WinMLTraceLoggingWrite(
      provider_,
